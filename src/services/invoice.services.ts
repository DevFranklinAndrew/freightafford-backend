...
export const sendUploadInvoiceNotification = async (
  email: string,
  fullname: string,
  ...
  return await resend.emails.send({
    ...
    html: SEND_UPLOAD_INVOICE_BY_ADMIN.replace(
      "{{USERNAME}}",
...
export const sendSubmitPaymentProofNotification = async (
  adminEmail: string[],
  invoiceNumber: string,
  payReference: string,
  status: string,
  ...
  return await resend.emails.send({
    ...
    html: SEND_PAYMENT_PROOF_TO_ADMIN.replace(
      "{{INVOICE_NUMBER}}",
      invoiceNumber,
    )
...
export const sendVerifyPaymentNotification = async (
  email: string,
  fullname: string,
  ...
  return await resend.emails.send({
    ...
    html: VERIFY_PAYMENT_BY_ADMIN_TO_CUSTOMER.replace(
      "{{USERNAME}}",
      fullname.split(" ")[0] as string,
    )
