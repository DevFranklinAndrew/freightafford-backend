...
export const createTrackingEvent = async ({
  ...
  if (event) await Booking.findByIdAndUpdate(bookingId, { status: event });

  return trackingEvent;
};

export const getBookingTrackingEvents = async (
...
export const deleteTrackingEvent = async (
  ...
  if (!event) return next(new AppError("Tracking event not found", 404));
