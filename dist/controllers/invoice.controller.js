...
export const uploadInvoice = async (req, res, next) => {
    ...
    if (!["confirmed", "in_transit", "arrived", "delivered"].includes(booking.status))
        return next(new AppError("Invoice can only be uploaded for confirmed bookings", 400));
    ...
    const uploaded = await uploadToCloudinary(req.file, "invoices");
    if (!uploaded)
        return next(new AppError("File upload failed", 500));
    const freightRequest = booking.freightRequest;
    ...
    const { error } = await sendUploadInvoiceNotification(customer.email, customer.fullname, invoice.invoiceNumber, invoice.amount, invoice.dueDate);
    if (error)
...
export const getInvoiceByCustomer = async (req, res, next) => {
    const userId = req.user._id;
...
export const submitPaymentProof = async (req, res, next) => {
    ...
    if (invoice.status !== "pending")
        return next(new AppError("Payment already submitted or invoice not payable", 400));
    if (!paymentReference)
...
export const verifyPayment = async (req, res, next) => {
    ...
    const { error } = await sendVerifyPaymentNotification(customer.email, customer.fullname, action, invoice.invoiceNumber, invoice.status);
    if (error)
        return next(new AppError("Unable to send verify payment notification", 400));
    await invoice.save();
    res.status(200).json({
        status: "success",
