...
import { passwordResetEmail, resendOTPEmail, sendOTPEmail, } from "../services/auth.email.js";
import { generateJWT } from "../utils/jwt.js";
import { generateOTP, hashToken } from "../utils/otp.js";
import { comparePassword, hashPassword } from "../utils/password.js";
import { formatPhoneNumber, normalizePhoneNumber, } from "../utils/phoneFormat.js";
const MAX_OTP_ATTEMPTS = 5;
const MAX_RESEND_LIMIT = 3;
export const register = async (req, res, next) => {
    const { fullname, email, password, acceptTerms, acceptedFreightRules } = req.body;
    ...
    if (error)
        return next(new AppError("Unable to send verification mail! Kindly retry your registration.", 400));
    await user.save();
    res.status(201).json({
        status: "success",
...
export const resendOTP = async (req, res, next) => {
    ...
    await user.save();
    const { error } = await resendOTPEmail(user.email, user.fullname, otp, expiryMinutes);
    if (error)
        return next(new AppError("Unable to send verification mail! Kindly retry your registration.", 400));
    res
        .status(200)
...
export const getMe = async (req, res, next) => {
    const me = req.user;
    if (me.phoneNumber)
        me.phoneNumber = formatPhoneNumber(me.phoneNumber);
    if (!me)
...
export const forgotPassword = async (req, res, next) => {
    ...
    if (!user)
        return next(new AppError("Provide an existing account.", 404));
    ...
    const { error } = await passwordResetEmail(email, user.fullname, resetLink, expiryMinutes);
    if (error)
...
export const updateUserProfile = async (req, res, next) => {
    const { fullname, phoneNumber, country, companyName, companyAddress } = req.body;
    ...
    const update = await User.findByIdAndUpdate(user._id, {
        ...
        companyName,
        companyAddress,
    }, { new: true, runValidators: true }).select("-password -otp -passwordResetToken");
    res.status(200).json({
        status: "success",
